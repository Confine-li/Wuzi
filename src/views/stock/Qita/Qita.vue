	  ...
	  methods:{
		  querySearchAsync(queryString, cb) {
		       ...
		       async handleSelect(item) {
					const res = await this.$http.get(`rest/qita/select/${item.value}`)
					const id = res.data[0]._id
				    this.$router.push(`/qita/${id}`)
		        },
		  dianji(row, column, event){
			  console.log('row, column, event',row.id)
			  this.$router.push(`/qita/${row._id}`)
		  },
		  async fetch(){
				const res = await this.$http.get("rest/qitas")
				const res2 = await this.$http.get("rest/partqitas")
				// console.log('timeFinal',this.items.data2)
				this.items = res.data
				this.part = res2.data
				for(var i = 0; i<this.part.length;i++){
					this.filters.push({ text: this.part[i].qitaName, value: this.part[i].qitaName });
				}
		...
		async remove(row) {
			if(window.localStorage.shenfen == '管理员'){
				this.$confirm(`是否删除"${row.purchaseName}"？`, '提示', {
					...
					const res = await this.$http.delete(`rest/qitas/${row._id}`)
					this.$message({
						type: 'success',
						message: '删除成功!'
					});
			...
			filterHandler(value, row, column) {
				...
				this.title = value
				console.log('1',row[property][0].qitaName)
			}
	  },
	  created(){
		  this.fetch();
