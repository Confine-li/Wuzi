	  ...
	  <el-descriptions class="margin-top" :title="'设备>'+items.purchaseName+'设备详情(编号:'+items.purchaseId+')'" :column="2" border
		...
		<el-descriptions-item>
		  <template slot="label">
			...
			<el-image v-if="items.purchaseIcon" :src="items.purchaseIcon" class="avatar" :preview-src-list="srcList"/></el-image>
		</el-descriptions-item>
	  </el-descriptions>
...
export default {
	...
	methods: {
		async save() {
				 const res2 = await this.$http.put(`rest/qitas/${this.id}`,this.items)
      ...
      return '';
		...
		async stockList(){
				const res2 = await this.$http.get(`rest/ExQitas/parent/${this.id}`)
				this.list = res2.data;
				console.log('ExEquipment',res2.data)
			},
			async OutOfList(){
				const res2 = await this.$http.get(`rest/Everqitas/parent/${this.id}`)
				this.list2 = res2.data;
				console.log('ExEquipmentEvers',res2.data)
			},
			async fetch() {
				const res = await this.$http.get(`rest/qitas/${this.id}`);
				 this.items = res.data;
				this.items = res.data;
				this.srcList[0]= this.items.purchaseIcon
				}
		},
		created() {
			this.fetch();
