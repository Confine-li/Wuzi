	 ...
	 props: {
	  ...
	  methods:{
		       ...
		       async handleSelect(item) {
					const res = await this.$http.get(`rest/chanpin/select/${item.value}`)
					const id = res.data[0]._id
				    this.$router.push(`/chanpin/${id}`)
		        },
		  dianji(row, column, event){
			  console.log('row, column, event',row.id)
			  this.$router.push(`/chanpin/${row._id}`)
		  },
		  async fetch(){
				const res = await this.$http.get("rest/chanpin")
				const res2 = await this.$http.get("rest/partchanpins")
				// console.log('timeFinal',this.items.data2)
				this.items = res.data
				this.part = res2.data
				for(var i = 0; i<this.part.length;i++){
					this.filters.push({ text: this.part[i].chanpinName, value: this.part[i].chanpinName });
				}
		...
		async remove(row) {
			if(window.localStorage.shenfen == '管理员'){
				this.$confirm(`是否删除"${row.purchaseName}"？`, '提示', {
					...
					const res = await this.$http.delete(`rest/chanpin/${row._id}`)
					this.$message({
						type: 'success',
						message: '删除成功!'
					});
			...
			filterChange (filterObj) { 
				 filterObj = filterObj.chanpinName
			...
			filterHandler(value, row, column) {
				...
				this.title = value
				console.log('1',row[property][0].chanpinName)
			}
	  },
	  created(){
		  this.fetch();
