...
export default {
	...
	methods: {
		async save() {
				 const res2 = await this.$http.put(`rest/shebei/${this.id}`,this.items)
      ...
      return '';
		...
		async stockList(){
				const res2 = await this.$http.get(`rest/ExShebei/parent/${this.id}`)
				this.list = res2.data;
			},
			async OutOfList(){
				const res2 = await this.$http.get(`rest/Evershebei/parent/${this.id}`)
				this.list2 = res2.data;
			},
			async fetch() {
				const res = await this.$http.get(`rest/shebei/${this.id}`);
				 this.items = res.data;
