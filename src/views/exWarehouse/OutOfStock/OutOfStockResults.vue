<template>
	...
	>
		<el-container style="width: 800px; margin: 0 auto;">
			<h1 style="margin: 30px auto;">物资出库清单</h1>
			<el-descriptions :column="2" :size="size" border>
				...
				<el-descriptions-item>
					<template slot="label">
						<!-- <i class="el-icon-user">1</i> -->
						<p style="width: 80px;">出库时物资状态</p>
					</template>
					<p style="width:200px;">{{ items.exSituation }}</p>
				</el-descriptions-item>
				<el-descriptions-item>
					<template slot="label">
						<p style="width:100px;">出库图片</p>
					</template>
					<template>
						<template>
				...
				<el-descriptions-item :span="2">
					...
					<p style="width:200px;" v-if="items.text!=undefined">
						{{ items.text }}
						<span>{{ items.returnTime }}</span>
					</p>
					<el-input type="textarea" v-model="items.text" v-else :disabled="inputable"></el-input>
				</el-descriptions-item>
				</el-descriptions>
			<el-footer>
...
export default {
	...
	methods: {
		...
		if (this.index==1) {
			let res = await this.$http.put(`rest/Everchanpin/${this.id}`, this.items);
			...
			let res = await this.$http.put(`rest/Everwuliao/${this.id}`, this.items);
			...
			let res = await this.$http.put(`rest/Evershebei/${this.id}`, this.items);
			...
			let res = await this.$http.put(`rest/Everqitas/${this.id}`, this.items);
		...
		async fetch() {
			const res1 = await this.$http.get(`rest/Everchanpin/${this.id}`);
			const res2 = await this.$http.get(`rest/Everwuliao/${this.id}`);
			const res3 = await this.$http.get(`rest/Evershebei/${this.id}`);
			const res4 = await this.$http.get(`rest/Everqitas/${this.id}`);
			if (res1.data.length != []) {
				this.index = '1';
